...
import { fetchClassById, fetchCourseById, fetchDepartmentById, fetchStatusById, registerClass } from '../../../services/UserServices';
...
const ListClass = () => {
    ...
    const handleConfirm = async () => {
        const data = {}
        const res = await registerClass(dataClass.id,data,localStorage.getItem('access_token'))
        console.log('res >>>',res)
    }
    return (
        <div className='body m-h-1250'>
            <div className='background h-68'>
            <div className='content'>
            <div>
            <div className='link-resource'><Link to='/Grade'>Danh sách lớp học</Link> / {dataClass.className}</div>
            <div className='content-class-detail h-565'>
            ...
            <div className='end-body-list-class'>
                ...
                <div className='btn-duyet-list-class' onClick={() => handleClickAccess()}>Đăng Ký</div>
            </div>
        </div>
            </div>
            </div>
