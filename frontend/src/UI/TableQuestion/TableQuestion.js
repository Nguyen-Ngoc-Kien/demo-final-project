...
import {fetchQuestionById} from '../../services/UserServices';
import ReactPaginate from 'react-paginate';
import { ToastContainer, toast } from 'react-toastify';
import ModalEditUser from './ModalEditQuestion';
...
const handleDeleteUser = (user) => {
  ...
  const getUsers = async (page) => {
    let res = await fetchQuestionById(localStorage.getItem("TopicId"),localStorage.getItem("access_token"));
  ...
  return(
    ...
    <Link to='/manager-questions'>
      <button className='btn btn-success d-flex justify-spacebetween btn-addnewuser'>Add new Question</button>
    </Link>
          ...
          {listUser && listUser.length > 0 &&
            listUser.map((item,index) => {
              ...
              return(
                <tr key={`users-${index}`}>
                  <td>{item.id}</td>
                  <td className='q-name'>{item.questionName}</td>
                    <td>
                    <Link to='/detail-question'>
                      <i class="far fa-eye icon-td-5" onClick={() => handleClickEye(item.id)}></i>
                    </Link>
                    </td>
                  <td>
