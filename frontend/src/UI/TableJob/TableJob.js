...
import {fetchAllJob} from '../../services/UserServices';
import ReactPaginate from 'react-paginate';
import ModalAddnew from './ModalAddnew';
import { ToastContainer, toast } from 'react-toastify';
import ModalEditUser from './ModalEditJob';
...
const handleDeleteUser = (user) => {
  ...
  return(
    <div className='container mt-5'>
          <button className='btn btn-success d-flex justify-spacebetween btn-addnewuser' onClick={() => setIsShowModalAddNew(true)}>Add new job</button>
          <div>
            <input 
            type='text'
          ...
          {listUser && listUser.length > 0 &&
            listUser.map((item,index) => {
              ...
              return(
                <tr key={`users-${index}`}>
                  <td>{item.id}</td>
                  <td className='level-w'>{item.jobName}</td>
