...
import { postQuestions } from '../../services/UserServices';
...
const Questions = (props) => {
    ...
    const handleSelectQuestion = (questionId,event) => {
        ...
        if(index > -1){
            questionClone[index].level = event.value
        }
        setQuestions(questionClone)
    }

    const handleOnChangeInput = (questionId,value) => {
        let questionClone = _.cloneDeep(questions)
        let index = questionClone.findIndex(item => item.id === questionId)
        if(index > -1) {
            questionClone[index].topicId = value
        }
        setQuestions(questionClone)
    }
    ...
    const handleSaveQuestions = async () => {
        ...
        const createdQuestion = questions.map(async (question,index) => {
            console.log("question >>>>>",question.topicId)
        ...
        <div className="question-container">
            ...
            && questions.map((question,index) => {
                ...
                <div className='form-floating mb-3 col-6'>
                        ...
                        onChange={(event) => handleOnChangeInput(question.id,event.target.value)}
                        />
                        <label className='q-name'>Topic Id Question {index + 1}</label>
